...
#------------------------------------------------------------------------------#
CrudeMetricsFigure.OTHER <- function(crudeMetrics, dateType, scaleY.input, 
                               ...
                               showBarNum.input, barNumSize.input, dotSize.input) {
...
# unique values to loop through (i.e., location, calibration, type).           #
# Additionally, it cleans up the dates in the data and creates the indicator   #
# to determine which figure to produce.                                        #
#------------------------------------------------------------------------------#
  ...
  ########################
  typeUnique <- unique(crude.metric.input.TP$Type)
  ...
  #########################################
  indexPoint <- length(locationUnique) * length(calibrationUnique) * length(typeUnique)
...
#------------------------------------------------------------------------------#
  ...
  for(i in 1:length(locationUnique)){
    ...
    for(c in 1:length(calibrationUnique)){
      ...
      #################################
      # Starting the metric type loop #
      #################################
      for(m in 1:length(typeUnique)){
      ...
      indexedData <- data %>%
        dplyr::filter(Location == locationUnique[i], # Filtering locations
                      ...
                      Type == typeUnique[m]) %>% # Filtering Type
      ...
      ##########################
      if(length(unique(indexedData$Date)) == 1){
...
#------------------------------------------------------------------------------#
      ...
      if(singleGraph == 1){
        ...
        #############################
        titleFigure <- paste0(typeUnique[m], "-", locationUnique[i], "-Calibration-", calibrationUnique[c], "-", max(finalData$Date))
...
#------------------------------------------------------------------------------#
          ...
          ######################################
          MSEFig <- ggplot(data = finalData, aes(x = dates, y = MSE, color = Model), size = dotSize) +
          ...
          ######################################
          MAEFig <- ggplot(data = finalData, aes(x = dates, y = MAE, color = Model), size = dotSize) +
            geom_point(position = position_jitter(width = 2, height = 0)) + 
          ...
          ######################################
          WISFig <- ggplot(data = finalData, aes(x = dates, y = WIS, color = Model), size = dotSize) +
            geom_point(position = position_jitter(width = 2, height = 0)) +
          ...
          #####################################
          PIFig <- ggplot(data = finalData, aes(x = dates, y = PI, color = Model), size = dotSize) +
            geom_point(position = position_jitter(width = 2, height = 0)) +
          ...
          # Creating the figure title #
          #############################
          titleFigure <- paste0(typeUnique[m], "-", locationUnique[i], "-Calibration-", calibrationUnique[c])
...
#------------------------------------------------------------------------------#
  ...
  # Removing NULL
  finalList <- figureFinalList[!sapply(figureFinalList, is.null)]
